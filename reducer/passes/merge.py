...
class MergeOnPathPass(AbstractPass):
    ...
    def transform(self, cfg : CFG, path : Path) -> tuple[CFG, Path]:
        ...
        for i in range(n):
            ...
            new_path = self.update_path(new_cfg, cfg, path, node1, node2)
    ...
    def update_path(self, cfg : CFG, old_cfg : CFG, old_path : Path, node1 : int, node2 : int) -> Path:
        ...
        for i in node_indices:
            ...
            # replace out-direction
            full_direction_array[i] = self.get_new_direction_using_adj(
                    cfg,
                    new_path.expected_output[i],
                    new_path.expected_output[i+1]
                    )

            #print(f'replace out direction of node {new_path.expected_output[i]} to {new_path.expected_output[i+1]} with dir {full_direction_array[i]}')

            # also replace in-direction if it is node1
            if new_path.expected_output[i] == node1:
                full_direction_array[i-1] = self.get_new_direction_using_adj(
                        cfg, 
                        new_path.expected_output[i-1], 
                        node1
                        )
    ...
    def get_new_direction_using_adj(self, new_cfg : CFG, u : int, v : int) -> int:
    ...
    def get_new_direction_using_edges(self, new_cfg : CFG, u : int, v : int) -> int:
